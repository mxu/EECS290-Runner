...
public class Grapher4 : MonoBehaviour {
	...
	*/
	...
	void Update() {
		...
		MethodInfo f = this.GetType ().GetMethod (functions [(int)function], BindingFlags.Static);
		float t = Time.timeSinceLevelLoad;
		for (int i = 0; i < points.Length; i++) {
			Color c = points[i].color;
			object[] paramArray = {points[i].position, t};
			if(absolute) {
				c.a = (float)f.Invoke(null, paramArray) >= threshold ? 1f : 0f;
			} else {
				c.a = (float)f.Invoke(null, paramArray);
			}
			points[i].color = c;
		}
